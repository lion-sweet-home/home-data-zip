...
export default function RentWolseCard({ item, rank = 1, onClick }) {
  const name = item?.aptName || '아파트';
  const dong = item?.dong || '';
  const exclusive = item?.exclusive != null ? Number(item.exclusive) : null;
  const avgDeposit = item?.avgDeposit;
  const avgRent = item?.avgMonthRent;
  const count = item?.wolseCount ?? 0;

  // 월세는 월세 등락률 우선, 없으면 보증금 등락률 표시
  const rate = formatRate(item?.monthRentChangeRate ?? item?.depositChangeRate);
    ...
    >
      ...
      <div className="mt-4 grid grid-cols-2 gap-y-2 text-sm">
        ...
        <div className="text-right font-semibold text-blue-700">
          {formatManwon(avgDeposit)} / {formatManwon(avgRent)}{' '}
          <span className="text-gray-800 font-medium">원</span>
        </div>
