...
export default function SaleCard({ item, rank = 1, onClick }) {
  const name = item?.aptName || '아파트';
  const dong = item?.dong || '';
  const exclusive = item?.exclusive != null ? Number(item.exclusive) : null;
  const avg = item?.avgDealAmount;
  const count = item?.saleCount ?? 0;
  const rate = formatRate(item?.dealAmountChangeRate);
