...
public interface UserNotificationRepository extends JpaRepository<UserNotification, Long> {
    ...
    @Query("SELECT un FROM UserNotification un WHERE un.user.id = :userId AND un.isDeleted = false ORDER BY un.createdAt DESC")
    List<UserNotification> findAllByUserId(@Param("userId") Long userId);

    // 읽음 목록 조회 (readAt이 null이 아닌 것, 최신순)
    @Query("SELECT un FROM UserNotification un WHERE un.user.id = :userId AND un.readAt IS NOT NULL AND un.isDeleted = false ORDER BY un.createdAt DESC")
    List<UserNotification> findReadByUserId(@Param("userId") Long userId);

    // 미읽음 목록 조회 (readAt이 null인 것, 최신순)
    @Query("SELECT un FROM UserNotification un WHERE un.user.id = :userId AND un.readAt IS NULL AND un.isDeleted = false ORDER BY un.createdAt DESC")
