...
public class JeonseMonthAvgTop3RepositoryImpl implements JeonseMonthAvgTop3Repository {
    ...
    @Override
    public List<MonthTop3JeonsePriceResponse> top3ByLastMonth(YearMonth lastMonth) {
        ...
        NumberExpression<Integer> lastCountSum = m.jeonseCount.sum();
        NumberExpression<Long> lastDepositSum = m.jeonseDepositSum.sum();
        ...
        NumberExpression<Integer> prevCountSum = m.jeonseCount.sum();
        NumberExpression<Long> prevDepositSum = m.jeonseDepositSum.sum();
        ...
        List<MonthTop3JeonsePriceResponse> result = new ArrayList<>();

        for (AptMonthAggRow last : lastTop3) {
            PrevAggRow prev = prevByAreaTypeId.get(last.areaTypeId());

            Long lastAvg = safeAvg(last.depositSum(), last.countSum());
            Long prevAvg = (prev == null) ? null : safeAvg(prev.depositSum, prev.countSum);

            Double changeRate = safeChangeRate(lastAvg, prevAvg);

            result.add(new MonthTop3JeonsePriceResponse(
                    last.aptId(),
                    last.exclusive(),
                    last.aptName(),
                    last.gugun(),
